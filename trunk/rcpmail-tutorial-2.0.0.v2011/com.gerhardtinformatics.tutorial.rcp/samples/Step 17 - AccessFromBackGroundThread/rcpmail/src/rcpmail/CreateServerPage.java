...
public class CreateServerPage extends WizardPage {
	...
	public static class NotEmptyValidator implements IValidator {
		...
		public IStatus validate(Object value) {
			...
			if (string == null || string.length() == 0) {
				return ValidationStatus
				.error("Please enter a value for " + fieldname + ".");
			}
			return ValidationStatus.ok();
		}
	...
	public void createControl(Composite parent) {
		...
		dbc.bindValue(
				...
				new UpdateValueStrategy().setAfterConvertValidator(new NotEmptyValidator("username")), null);
		dbc.bindValue(
		...
		// create this after binding the observables to avoid displaying an error when the
