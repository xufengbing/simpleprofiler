...
var Slideous = {
   ...
   insertlinks: function() {
     ...
     for (var i=0; i<refs.length; i++) {
       if (refs[i].innerHTML && refs[i].href.match(/http:/i)) {
         refs[i].innerHTML = refs[i].innerHTML + "<span class=\"footnote\"> [" + counter + "]</span>";
	     list += "["+ counter++ + "] " + refs[i].href  + "<br/>";
       }
     }
