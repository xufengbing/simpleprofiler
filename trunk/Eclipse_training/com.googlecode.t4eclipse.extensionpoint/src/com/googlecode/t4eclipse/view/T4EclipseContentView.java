...
public class T4EclipseContentView extends ViewPart {
	...
	private void createViewer(Composite parent) {
		...
		ColumnViewerToolTipSupport.enableFor(tview);
		// using tooltip support need to use CellLabelProvider
		tview.getTree().setLinesVisible(true);
		tview.getTree().setHeaderVisible(true);
	...
	private Composite createComposite(TreeModel model) {
		try {
			...
			Bundle bundle = Platform.getBundle(model.getPlugInID());
			Class s = bundle.loadClass(model.getClassID());
